...
void initReceiver() {
  ...
  for (int i = 0; i < NB_MAX_ROBOTS; i++) {
    memset(&newComingRobot, 0, sizeof(newComingRobot));
    newComingRobot.id = i + receiverId * NB_MAX_ROBOTS;
...
void handleIncomingUSBMessage() {
  ...
  if (length) {
    //handshakes handling
    if ((!managerListening) || (length == (sizeof(HANDSHAKE_REQUEST))) || (length == (sizeof(HANDSHAKE_LEAVE)))) {
      if (strcmp(tmpBuffer, HANDSHAKE_REQUEST) == 0) {
        writeUSB(sizeof(HANDSHAKE_REPLY), HANDSHAKE_REPLY);
        ...
        if (DEBUG_ENABLED())
          debug_printf("Manager listening\n");
        else
          setStatusLed(200);
      } else if (strcmp(tmpBuffer, HANDSHAKE_LEAVE) == 0) {
        managerListening = false;
        if (DEBUG_ENABLED())
          debug_printf("Manager left\n");
        else
          setStatusLed(0);
      }
    }
      ...
      if ((payloadSize + 4) < usbBufferIn.count) {
        if (RingBuffer_get(&usbBufferIn, &tmpInBuffer, 4 + payloadSize) == true && tmpInBuffer == '!') {
          if (recipientId == RECEIVER_ID) {
            if (messageType == TYPE_REBOOT_ROBOT)
              Reboot();
          } 
          else if ((recipientId - NB_MAX_ROBOTS * receiverId) < myZooids.count) {
            Robot *r = RobotRingBuffer_get(&myZooids, (recipientId - NB_MAX_ROBOTS * receiverId));
            if (r) {
