...
bool initNRF24L01P()
{
    ...
    initOK &= setRetries(0, 2); //(1+1)x250us delay 2 tries
...
//    clearInterruptFlag(true, true, false);
...
{
    uint8_t payloadSize = 0xff;
    SPIReadByte(R_RX_PL_WID, &payloadSize);
    return payloadSize;
}
...
bool enableAckPayload(void)
{
    ...
    return ((readRadioRegister(FEATURE) == features && readRadioRegister(DYNPD) == dynpd) ? true : false);
}
...
{
    bool result;
    result = SPIWriteCommand(FLUSH_RX);
    return result;
}
...
{
    bool result;
    result = SPIWriteCommand(FLUSH_TX);
    return result;
}
...
{
    return setChannel(RADIO_CHANNEL);
}
/****************************************************************************/
