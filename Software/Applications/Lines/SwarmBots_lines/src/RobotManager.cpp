...
//--------------------------------------------------------------
bool RobotManager::updateRobot(unsigned int id, ofVec2f position, float orientation, ofColor color, bool reassignable) {
	if (id < robots.size())
	{
		robots[id].setGoal(position);
		robots[id].setOrientation(orientation);
		robots[id].setColor(color);
        robots[id].setReassignable(reassignable);
		return true;
	}
	return false;
}
			...
			for (int i = 0; i < nbBytesReceived / sizeof(RobotState); i++)
			{
				...
				for (int i = 0; i<robots.size(); i++)
				{
					...
					if (it != robots.end())
					{
                        ...
                        it->setReassignable(tmpRobot.isReassignable());
						foundRobot = true;
						break;
					}
				}
...
//--------------------------------------------------------------
RobotState::RobotState(unsigned int _id, ofVec2f _position, ofVec2f _goal, ofColor _color, bool _reassignable) {
	position = _position;
	radius = 0.01f;
	orientation = 0.0f;
	goal = _goal;
...
//--------------------------------------------------------------
bool RobotState::isReassignable() {
    return reassignable;
}

//--------------------------------------------------------------
void RobotState::setReassignable(bool _reassignable) {
    reassignable = _reassignable;
}
