...
public:
    RobotState();
    ~RobotState();
	RobotState(unsigned int _id, float _radius, ofVec2f _position, ofVec2f _goal, float _angle, ofColor _color, uint8_t _battery, bool _reassignable);
    ...
    void setReassignable(bool r);

    void drawRobot(ofxSVG* artwork, float scale, bool debug);
	void drawGoal();
