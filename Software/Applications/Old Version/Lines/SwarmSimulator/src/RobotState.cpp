...
RobotState::RobotState(unsigned int _id, float _radius, ofVec2f _position, ofVec2f _goal, float _angle, ofColor _color, uint8_t _battery, bool _reassignable) {
	position = _position;
	radius = _radius;
	angle = _angle;
	goal = _goal;
...
void RobotState::setReassignable(bool r) {
    reassignable = r;
}
...
void RobotState::drawRobot(ofxSVG* artwork, float scale, bool debug)
{
	ofPushMatrix();
    ...
    if (debug) {
        ...
        ofFill();
        ofSetColor(color);
        ofDrawCircle(-0.001f, 0.0025f, 0.0025*scale);
    }
