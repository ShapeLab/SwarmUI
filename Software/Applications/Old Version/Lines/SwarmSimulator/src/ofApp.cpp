...
//    swarmReceivers.push_back(new SwarmReceiver(2));
    ...
    for (int i = 0; i < swarmReceivers.size() * NB_ROBOTS_PER_RECEIVER; i++)
    {
        ...
        float tmpAngle = ofRandom(-180.0f, 180.0f);
        
        ofColor tmpColor =  ofColor(ofRandomf()*255.0f, ofRandomf()*255.0f, ofRandomf()*255.0f);
        
        RobotState tmpRobot = RobotState(0, robotRadius, initialPosition, ofVec2f(0.0f, 0.0f), tmpAngle, tmpColor, 100, true);
...
void ofApp::draw(){
    ...
    for (int i = 0; i < robotCollection.size(); i++)
    {
        ...
        robotCollection[i].drawRobot(&robotArtwork, 1.0f, displayDebugMode);
        if (displayDebugMode)
    ...
    if(displayHelp)
    {
        ...
        "Press D to toggle debug display mode" << endl <<
...
bool ofApp::runSimulation()
{
    ...
    if (simulationMode == ON && (currentTimestep - previousSimulationTimestamp) >= timeStep * 500)
    {
        ...
        for (int i = 0; i < robotCollection.size(); i++)
        {
            ...
            simulator.setAgentPrefSpeed(robotCollection[i].getId(), prefSpeed * k);
            simulator.setAgentMaxSpeed(robotCollection[i].getId(), 1.1f * prefSpeed * k);
            
            if (simulator.getAgentReachedGoal(robotCollection[i].getId()))
                simulator.setAgentPrefSpeed(robotCollection[i].getId(), 0.0f);
            else
                simulator.setAgentPrefSpeed(robotCollection[i].getId(), prefSpeed * k);
...
void ofApp::readRobotsPositions()
{
    ...
    for (int i = 0; i < swarmReceivers.size(); i++)
    {
        while (swarmReceivers[i]->availableMessages() > 0)
        {
            ...
            if (msg.getSenderId() < robotCollection.size() && msg.getType() == TYPE_STATUS)
            {
                ...
                int index = 0;
                for (int i=0; i<robotCollection.size(); i++)
                    if (robotCollection[i].getId() == msg.getSenderId()) index = i;
                
                robotCollection[index].setTouch(status.touch);
                robotCollection[index].setPosition(robotX, robotY);
                //robotCollection[index].setAngle(robotA);
                robotCollection[index].setBatteryLevel(status.batteryLevel);
            }
        }
    }
}
...
void ofApp::controlRobotPosition(uint8_t id, float x, float y, ofColor color, float orientation, float preferredSpeed, bool isFinalGoal)
{
    ...
    if (id >= 0 && id < robotCollection.size())
    {
        ...
        swarmReceivers[receiverId]->sendUSB(TYPE_ROBOT_POSITION, robotCollection[id].getId(), sizeof(msg), (uint8_t*)&msg);
    }
}
...
void ofApp::receiveAppGoals()
{
    ...
    while (udpReceiver.PeekReceive()>0)
    {
        ...
        for (int j = 0; j<nbBytesReceived / sizeof(RobotState); j++)
        {
            ...
            if (tmpRobot.getGoal().x < 2.0f * robotDiameter) tmpRobot.setGoal(2.0f * robotDiameter, tmpRobot.getGoal().y);
            if (tmpRobot.getGoal().x > dimensionX - 2.0f * robotDiameter) tmpRobot.setGoal(dimensionX - 2.0f * robotDiameter, tmpRobot.getGoal().y);
            if (tmpRobot.getGoal().y < 2.0f * robotDiameter) tmpRobot.setGoal(tmpRobot.getGoal().x, 2.0f *  robotDiameter);
            if (tmpRobot.getGoal().y > dimensionY - 2.0f * robotDiameter) tmpRobot.setGoal(tmpRobot.getGoal().x, dimensionY - 2.0f * robotDiameter);
            //            for (int i = 0; i<robotCollection.size(); i++)
            //            {
            ...
            //                auto it = robotCollection
            //                if (it != robotCollection.end())
            //                {
            robotCollection[j].setAngle(tmpRobot.getAngle());
            robotCollection[j].setColor(tmpRobot.getColor());
            robotCollection[j].setReassignable(tmpRobot.isReassignable());
            //                    break;
            //                }
            //            }
            ...
            if(j<simulator.getNumGoals())
                simulator.setAgentGoal(robotCollection[j].getId(), hrvo::Vector2(tmpRobot.getGoal().x, tmpRobot.getGoal().y));
        }
...
void ofApp::assignGoalIndex(unsigned char mode) {
    ...
    if (mode == 0) {
        for (int k = 0; k < robotCollection.size(); k++) {
            simulator.setAgentGoal(robotCollection[k].getId(), robotCollection[k].getId());
            robotCollection[k].setGoal(ofVec2f(simulator.getGoalPosition(robotCollection[k].getId()).getX(), simulator.getGoalPosition(robotCollection[k].getId()).getY()));
        }
    }
    else if (mode == 1) {
        ...
        for(int i=0; i<size; i++)
        {
            ...
            {
                if (robotCollection[i].getTouch() == 0 && robotCollection[i].isReassignable() && robotCollection[j].getTouch() == 0 && robotCollection[j].isReassignable())
                {
                    Cost[i][j] = pow(100.0f*(robotCollection[i].getPosition() - ofVec2f(simulator.getGoalPosition(robotCollection[j].getId()).getX(), simulator.getGoalPosition(robotCollection[j].getId()).getY())).length(), 2.0f);
                }
                else
                    Cost[i][j] = 10000000000.0f;
            }
        }
        ...
        // First, verify if the reassignment obeys the hard constraints. For some reason, it doesn't always.
        // If not, don't reassign this time.
        bool reassignmentIsValid = true;
        for (int i=0; i<assignment.size(); i++) {
            if(robotCollection[i].getTouch() || !robotCollection[i].isReassignable()) {
                if (robotCollection[i].getId() != robotCollection[assignment[i]].getId()) {
                    reassignmentIsValid = false;
                    break;
                }
            }
        }
        
        if (reassignmentIsValid) {
            
            // Reassign robot IDs across robots. Don't change the rest.
            
            vector<int> inverseAssignment;
            inverseAssignment.resize(assignment.size());
            for (int i = 0; i < assignment.size(); i++)
                inverseAssignment[assignment[i]] = i;
            
            vector<RobotState> newRobotCollection;
            newRobotCollection.resize(robotCollection.size());
            for (int r = 0; r < robotCollection.size(); r++) {
                RobotState newRobot = robotCollection[r];
                int old_r = inverseAssignment[r];
                newRobot.setPosition(robotCollection[old_r].getPosition());
                newRobot.setId(robotCollection[old_r].getId());
                newRobotCollection[r] = newRobot;
            }
        ...
        for (int k = 0; k < robotCollection.size(); k++)
        {
            // Do nothing special - the simulation is not aware of the reassignment
